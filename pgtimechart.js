...
export async function main(event, context, callback) {
    ...
    let query = "select decision_date, count(distinct(decision_no)) from planning_decisions pd, s_cadastral_land_lot_1 cada where decision_date >= date'" + s + "' and decision_date < date'" + e+ "' and pd.mkts_lotno = cada.lot_key and ST_Within(cada.geom, " + bounds + ") group by decision_date order by decision_date ASC";
